...
Checkpoint: `2026-02-21-skr-stable`
- Snapshot path: `wp-content/plugins/cargo-docs-studio/checkpoints/2026-02-21-skr-stable`
- Restore sources: `HtmlComposer.php`, `RenderPipeline.php`, `documents.js`, `README.md`, `TODO.md`
...
- [x] Add PDF delete action in generation result panel (post-generate cleanup)
...
- [x] Added GitHub release automation workflow (`.github/workflows/release.yml`) on `v*.*.*` tag push
- [x] Added plugin GitHub updater integration (stable public releases + release-asset ZIP contract)
- [x] Added repository bootstrapping files (`.gitignore`, `.gitattributes`, `LICENSE`, dependabot config)
- [x] Removed dead SKR legacy render path (`renderLegacy`) and kept stable SKR renderer only
- [x] Fixed receipt runtime bug by initializing `payment_network` before rendering
- [x] Removed invoice totals override section exposure (`tax_amount`, `insurance_amount`, `grand_total`)
- [x] Optimized invoice generation path to skip tracking QR image build while preserving API envelope
- [x] Optimized admin payload sync with cached form nodes and no full form round-trip on each keystroke
...
- [x] Tracking admin now supports browser geolocation auto-capture (`Use Current GPS`) with safe manual lat/lng fallback

## 7. Separation of Concerns Refactor - 100%
Progress: `[##############################]`

- [x] Phase 1: Split `HtmlComposer.php` into composer facade + per-document renderers + shared services
- [x] Keep `RenderPipeline` and `HtmlComposer::composeInvoice(...)` contracts stable
- [x] Phase 1: staging regression pass (invoice/receipt/SKR output parity matrix)
- [x] Phase 2: split `assets/admin/documents.js` into modules (state/defaults/sections/index + stable entry shim)
- [x] Phase 2: UI regression pass (form/json mode, list, delete from list/result)
- [x] Phase 3: cleanup dead paths and finalize architecture docs


- [x] Phase 2 tranche: extracted active validation/list/generation modules and wired enqueue/runtime

- [x] Phase 2 tranche: extracted form-renderer and payload-sync modules; index.js reduced to orchestration

- [x] Phase 3 prep: deduplicated shared JS helpers into documents/utils.js and reduced index.js complexity
