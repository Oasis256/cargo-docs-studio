...
class RenderPipeline
{
    ...
    public function generateDocumentPdf(string $docTypeKey, array $payload, array $trackingData): array
    {
        ...
        $resolvedWatermark = $this->resolveDocumentWatermarkUrl($payload, $docTypeKey);
        if ($resolvedWatermark !== '') {
            // For PDF rendering, keep watermark in engine layer only (avoids duplicate/broken top-left image markers).
            $payload['watermark_url'] = '';
            $payload['skr_watermark_url'] = '';
            $renderOptions['watermark_url'] = $resolvedWatermark;
        }
    ...
    public function generateInvoicePreviewPdf(array $templateConfig, array $payload = []): array
    {
        ...
        $renderOptionsWatermark = $this->resolveDocumentWatermarkUrl($payload, $docTypeKey);
        if ($renderOptionsWatermark !== '') {
            // For PDF rendering, keep watermark in engine layer only.
            $payload['watermark_url'] = '';
            $payload['skr_watermark_url'] = '';
        }
        $html = $this->composer->composeInvoice($payload, $trackingBlock, $paymentBlock, $templateConfig);
        ...
        $renderOptions = $this->buildRenderOptions($templateConfig);
        if ($renderOptionsWatermark !== '') {
            $renderOptions['watermark_url'] = $renderOptionsWatermark;
        }
    ...
    public function generateInvoicePreviewHtml(array $templateConfig, array $payload = []): array
    {
        ...
        $resolvedWatermark = $this->resolveDocumentWatermarkUrl($payload, $docTypeKey);
        if ($resolvedWatermark !== '') {
            $payload['watermark_url'] = $resolvedWatermark;
            $payload['skr_watermark_url'] = $resolvedWatermark;
        }
    ...
    private function resolveDocumentWatermarkUrl(array $payload, string $docTypeKey): string
    {
        $default = $this->settings->get('skr_watermark', ['url' => '']);
        $settingsUrl = esc_url_raw((string) ($default['url'] ?? ''));

        $enabled = array_key_exists('watermark_enabled', $payload)
            ? !empty($payload['watermark_enabled'])
            : false;

        // Backward compatibility with existing SKR payloads.
        if ($docTypeKey === 'skr' && !array_key_exists('watermark_enabled', $payload)) {
            $enabled = array_key_exists('skr_watermark_enabled', $payload)
                ? !empty($payload['skr_watermark_enabled'])
                : true;
        }
        ...
        // Legacy fallback for older payload JSON.
        return esc_url_raw((string) ($payload['skr_watermark_url'] ?? ($payload['watermark_url'] ?? '')));
    }
