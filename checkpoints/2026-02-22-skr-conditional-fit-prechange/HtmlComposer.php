...
<div class="wrap">
  ...
  ' . $body . '
    ...
    private function renderReferenceInvoice(array $payload, array $trackingBlock, ?array $paymentBlock, array $computed, array $theme): string
    {
        ...
        $logoUrl = $this->resolveImageSource((string) ($payload['company_logo_url'] ?? ''));
...
<table class="footer-table">
  <tr>
    <td style="width:60%;text-align:center;vertical-align:top;padding:10px;">
      <div style="margin-bottom:15px;"><strong>For and on behalf of 3</strong></div>
...
.tracking{text-align:center;color:#b71c1c;font-weight:700;font-size:10pt;margin:6px 0 3px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;}
...
<table class="sign">
  <tr>
    <td style="width:50%;"><br><br>Kimbugwe Faisal<br><strong>ISSUING OFFICER</strong></td>
    <td class="sign-right" style="width:50%;"><br><br>' . $todayPretty . '<br><strong>Official Stamp / Signature</strong></td>
  </tr>
</table>
</body>
</html>';
    ...
    private function renderReferenceSkrStable(array $payload, array $trackingBlock, array $theme): string
    {
        ...
        $issuerTitle = sanitize_text_field((string) ($payload['issuer_title'] ?? 'ISSUING OFFICER'));
        ...
        $displayStorageFees = $storageFeesLabel !== '' ? esc_html($storageFeesLabel) : ('PER DAY = $' . number_format($storageFees, 2));
        $trackingCode = sanitize_text_field((string) ($payload['tracking_code'] ?? $depositNumber));
        $trackingQrUri = esc_url_raw((string) ($trackingBlock['data_uri'] ?? ''));
        $trackingFontSize = $this->computeTrackingFontSize($trackingCode);
        $trackingFontSizeCss = rtrim(rtrim(number_format($trackingFontSize, 2, '.', ''), '0'), '.');
...
<table class="table">
  ...
  <tr><td><span class="red">' . esc_html($depositorName) . '</span></td><td>' . esc_html((string) ($payload['projected_days'] ?? 'N/A')) . '</td><td>' . esc_html($documentedCustomValue) . '</td></tr>
...
<table class="table">
  ...
  <tr><td><span class="red">' . esc_html($contentDescription) . '</span></td><td>' . esc_html($quantity) . ' ' . esc_html($unit) . '</td><td>' . esc_html($packagesNumber) . '</td><td>USD: ' . number_format($declaredValue, 2) . '</td><td>' . esc_html($originOfGoods) . '</td></tr>
...
<div class="tracking" style="font-size:' . esc_attr($trackingFontSizeCss) . 'pt;">TRACK YOUR CONSIGNMENT ON OUR OFFICIAL WEBSITE WITH THE TRACKING NUMBER ' . esc_html($trackingCode) . '</div>
' . ($affidavitText !== '' ? '<div style="font-size:10pt;margin:6px 0;">' . nl2br(esc_html($affidavitText)) . '</div>' : '') . '
...
<table style="width:100%;border-collapse:collapse;margin-top:8px;">
  <tr>
    <td style="width:50%;font-size:10pt;border:none;"><br><br><br>' . esc_html($issuerName) . '<br><br><br><strong style="border-top:2px solid #000;">' . esc_html($issuerTitle) . '<br><br><br></strong></td>
    <td style="width:50%;font-size:10pt;border:none;text-align:right;">' . $displayDateLabel . '<br><strong>' . esc_html($stampLabel) . '</strong></td>
  </tr>
</table>
</div>
</div>
    ...
    private function computeTrackingFontSize(string $trackingCode): float
    {
        ...
        // Shrink progressively for long tracking values to keep the line on a single row.
